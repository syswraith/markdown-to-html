...
class Parser:
    def __init__(self):
        ...
        r"""
        line_content        = image / progress_bar / h1 / h2 / h3 / blockquote / text / empty
        ...
        inline              = inline_code / bold_italic / bold / italic / link / wikilink / plain
        inline_code         = ~r"`[^`\n]+`"
        bold_italic         = "***" inline+ "***"
        bold                = "**" inline+ "**"
        italic              = "*" inline+ "*"
...
class Visitor(NodeVisitor):
    ...
    def visit_blockquote(self, node, children): return f"<blockquote>{''.join(flatten(children[1:]))}</blockquote>"
    def visit_text(self, node, children): return f"<p>{''.join(flatten(children))}</p>"
    def visit_empty(self, node, children): return ""

    def visit_bold_italic(self, node, children): return f"<b><i>{''.join(flatten(children[1:-1]))}</i></b>"
    def visit_bold(self, node, children): return f"<b>{''.join(flatten(children[1:-1]))}</b>"
    def visit_italic(self, node, children): return f"<i>{''.join(flatten(children[1:-1]))}</i>"
    def visit_inline_code(self, node, children): return f"<code>{node.text[1:-1]}</code>"
...
html_content.insert(0, '<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/sakura.css/css/sakura.css" type="text/css">')
